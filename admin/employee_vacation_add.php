...
//$employee = $_POST["selected_emp_id"];
...
if($selected_emp_id == "Select an employee"){
	...
	header ("Location: employeee_select.php?error_message=$message");
	exit();
}
...
function Ck_employees_off($emp_off_ttl){
  ...
  global $get_emp_off_sql,$hdr_addin;
    ...
    if($_SESSION["ses_offtype"] == "P"){
        if($count_number_off >= $emp_off_ttl){
            ...
            $error_on_page = 1;
            if($chk_sub == 1){
                $message[] = "There are already the alotted number of people off (".$emp_off_ttl.")
        ...
        if($ttl_off_hours >= $emp_off_ttl){
            ...
            $error_on_page = 1;
            if($chk_sub == 1){
                $message[] = "There are already the alotted number of hours used (".$emp_off_ttl.")
                    for <b>" . $day_of_wk . "</b> in the department. To override this issue,
                    check the \"Override number of people off check\" checkbox and resubmit.<br />";
            }else{
                $message[] = "There are already the alotted number of hours used (".$emp_off_ttl.")
...
if(isset($add_vacation)){
	...
	$conf_sql = @mysql_query("SELECT * FROM vf_config WHERE dept_id = '$emp_dept' AND " .
	...
	for ($i=0; $i<7; $i++) {
	  ...
	  if($hours[$i] != "" || $type[$i] != "Select Type"){
		...
		$get_emp_off = @mysql_query("SELECT * FROM vf_vacation WHERE date = '$submit_date[$i]' AND emp_id = '$selected_emp_id'");
		while($emp_off_array = @mysql_fetch_array($get_emp_off)){
            $current_employee = $emp_off_array["emp_id"];

            //CHECK IF THE EMPLOYEE ALREADY HAS REQUESTED THIS DATE. IF SO SET A SWITCH TO ON
            if($current_employee == $selected_emp_id){
	            //SET ERROR VARIABLE TO ON
				$error_on_page = 1;
        ...
        if($check_conf != "ON"){
            ...
            $get_emp_off_sql ="SELECT vf_vacation.hours,vf_vacation.emp_id FROM " .
		            ...
		            "AND vf_employee.enabled != 'N' AND vf_employee.sub_dept_id = '$emp_sub_dept' AND " .
		            "vf_vacation.emp_id = vf_employee.emp_id";
		            
            //USE AS A SWITCH TO GIVE FEEDBACK TO THE USER WHILE RUNNING THE FUNCTION
            $chk_sub = 1;
            //CALL THE FUNCTION
	...
	************************************************************************/
	if($error_on_page == 0){
		...
		$feedback = "<p style=\"margin-left:10;\"><font color=\"#FF0000\" size=\"5\"><b>**
	    	Time off updated for $emp_name! **</b></font><br />";
		...
		for ($i=0; $i<7; $i++) {
		  if($type[$i] != "Select Type"){
			...
			if($ValidChar != 0){
			    ...
			    $select_year = strtotime($day_of_wk);
				$year_select = selected_date($select_year);     
	            ...
	            VALUES
	            ('$selected_emp_id','$emp_dept','$day_of_wk','$hours[$i]','$type[$i]','$_SESSION[ses_emp_id]','$_SESSION[ses_emp_id]','$ck_if_ovrride','$cur_date_time','$year_select')";
	    ...
	    **************************************************************************/
        ...
        $hdr_addin = "<script language=\"javascript\">
			window.open('print_time_off_request.php?content=$new_win_content&header=$new_win_header&ename=$emp_name','newWind', \"width=450,height=400,toolbar=no,scrollbars=yes,resizable=no\")
		    </script>";
...
if(isset($selected_emp_id)){
    ...
    while($Result = @mysql_fetch_array($SQL)){
        ...
        $GetVac = @mysql_query("SELECT * FROM vf_emp_to_hours WHERE to_id = '$TypeID' " .
        					"AND year = '$summary_year' AND emp_id = '$selected_emp_id'");
		$Vac_Array = @mysql_fetch_array($GetVac);
		 ...
		 if($Earned != "" || $Valid_date != ""){
			...
			$Get_Scheduled = @mysql_query("SELECT * FROM vf_vacation WHERE " .
               		"to_id = '$TypeID' AND emp_id = '$selected_emp_id' AND year = '$summary_year'");
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-left: 10;">
  ...
  <tr>
    <td>
      <a href="javascript:directForm('<?PHP echo $selected_year; ?>','<?PHP echo $selected_emp_id; ?>','employee_menu.php')">Return to the employee menu</a>
	  <br />&nbsp;<br /><a href="javascript:directForm('<?PHP echo $selected_year; ?>','<?PHP echo $selected_emp_id; ?>','employee_select.php')">Select a different employee</a>
	 </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
    ...
    <td>2. Add the hours requested on the correct day of the week.</td>
  </tr>
  <tr>
    <td>3. Select the time off type from the drop down that corresponds
            ...
            <!--EMPLOYEE TABLE-->
	          <table bgcolor="#C0C0C0" border="0" cellpadding="0" cellspacing="0" bgcolor="#00FFFF" width="100%">
	            ...
	            <tr>
	              ...
	              <td align="left"><span style="background-color:white">&nbsp;<?PHP echo $emp_name; ?>&nbsp;</span></td>
	            </tr>
	            <tr>
	              <td>&nbsp;</td>
	              <td>
