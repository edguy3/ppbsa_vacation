...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Cell(100,10,"Submitted Time Report",0,0,'L');
...
$pdf->SetWidths(array(35,30,15,35,50,30),1);//Set column width
$pdf->SetBorders("1",0);
...
for ($a=0; $a<$weeks_to_display; $a++) 
{
    ...
    for ($i=0; $i<7; $i++) 
    {
		...
		$pdf->SetXY(85,$y);
		$pdf->Cell(35,6,"RELIEF",1,0,'C',1);
		$pdf->SetXY(120,$y);
		$pdf->Cell(50,6,"NOTES",1,0,'C',1);
		$pdf->SetXY(170,$y);
		$pdf->Cell(30,6,"SUBMITTED",1,0,'C',1);		
		$y = $y + 6;		
		$pdf->SetXY(5,$y);
		$pdf->SetFont('Arial','',8);	
        ...
        while($people_array = mysql_fetch_array($sql_people))
        {
			...
			{				
         		if($vac_deny == "Y")
		          $vac_note = "Time off denied";            		
				
			    $pdf->SetAligns(array("L","C","C","L","L","C"),1);//Set the alignment of each column
				$pdf->Row(array($emp_full_name,$vacation_description,$vac_hours,$vac_replacement,$vac_note,$date_entered),1,1);//Set value of each column, and 1 for border, 1 background fill					    
			}
